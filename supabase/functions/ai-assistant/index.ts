...
serve(async (req) => {
  ...
  try {
    ...
    console.log("KEY debug:", JSON.stringify({ exists: !!LOVABLE_API_KEY, len: LOVABLE_API_KEY?.length, first10: LOVABLE_API_KEY?.substring(0, 10), last5: LOVABLE_API_KEY?.substring((LOVABLE_API_KEY?.length || 0) - 5) }));
